...
def preanm_simulator(true_function="softplus", n=10000, x_lower=0, x_upper=2, noise_std=1, noise_dist="gaussian", train=True, device=torch.device("cpu"), a=torch.tensor([1.0, 1.2, 1.5]),noise_corr=0):
...
class InnerProductLayer(nn.Module):
    ...
    """
    def __init__(self, in_dim, noise_std=1.0):
        super().__init__()
        self.in_dim = in_dim
...
class Engressor(object):
    ...
    @torch.no_grad()
    def predict_g(self, x, custom_weight_vector=torch.tensor([1.2,1.5]), target="mean", sample_size=512):
