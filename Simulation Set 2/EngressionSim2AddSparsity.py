...
def engression(x, y, 
               ...
               device="cpu", standardize=True, verbose=True, l1_lambda=1e-4): 
    ...
    engressor = Engressor(in_dim=x.shape[1], out_dim=y.shape[1], 
                          ...
                          standardize=standardize, device=device, check_device=verbose, verbose=verbose, l1_lambda=l1_lambda)
...
class Engressor(object):
    ...
    def __init__(self, 
                 ...
                 device="cpu", check_device=True, verbose=True, l1_lambda=1e-4): 
        super().__init__()
        self.num_layer = num_layer
        self.hidden_dim = hidden_dim
        self.sigmoid = sigmoid
    ...
    def train_one_iter(self, x_batch, y_batch):
        ...
        l1_reg = 0.0
        if (self.l1_lambda > 0.0) and (self.model.inner_product.learnable_weights is not None):
            l1_reg = self.l1_lambda * self.model.inner_product.learnable_weights.abs().sum()

        total_loss = loss + l1_reg

        total_loss.backward()
        self.optimizer.step()
        self.tr_loss += total_loss.item()
...
def run_engression_simulations_sparsity(
    N_sim, 
    ...
    sample_size=512,device='cpu',sigmoid=False, l1_lambda=1e-4
):
    """
    ...
    for sim in range(N_sim):
        ...
        engressor = engression(
            ...
            batch_size=batch_size, device=device, sigmoid=sigmoid, l1_lambda=l1_lambda
        )
