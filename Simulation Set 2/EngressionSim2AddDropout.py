...
class Net(nn.Module):
    ...
    def __init__(self, in_dim=1, out_dim=1, num_layer=3, hidden_dim=100, noise_dim=100,
                 add_bn=True, sigmoid=False, out_act=None, noise_std=1.0, dropout_prop=0.2):
        super().__init__()
        self.in_dim = in_dim
        ...
        net = []
        # input layer
        net.append(nn.Linear(in_dim + noise_dim, hidden_dim))
        if add_bn:
            net.append(nn.BatchNorm1d(hidden_dim))
        net.append(nn.ReLU(inplace=True))
        # add dropout after first activation
        net.append(nn.Dropout(p=self.dropout_prop))  # dropout
...
class MyNet(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, num_layer=3, hidden_dim=100, 
                 add_bn=True, sigmoid=False, dropout_prop=0.2):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.num_layer = num_layer
        ...
        self.net_layer = Net(in_dim=2, out_dim=out_dim, num_layer=num_layer, hidden_dim=hidden_dim, add_bn=add_bn, sigmoid=sigmoid, out_act="relu", dropout_prop=dropout_prop)
...
def engression(x, y, 
               ...
               device="cpu", standardize=True, verbose=True, dropout_prop=0.2): 
    ...
    engressor = Engressor(in_dim=x.shape[1], out_dim=y.shape[1], 
                          ...
                          standardize=standardize, device=device, check_device=verbose, verbose=verbose, dropout_prop=dropout_prop)
...
class Engressor(object):
    ...
    def __init__(self, 
                 ...
                 device="cpu", check_device=True, verbose=True, dropout_prop=0.2): 
        super().__init__()
        self.num_layer = num_layer
        self.hidden_dim = hidden_dim
        self.sigmoid = sigmoid
        ...
        self.model = MyNet(in_dim, out_dim, num_layer, hidden_dim, add_bn, sigmoid, dropout_prop).to(self.device)
...
def run_engression_simulations_dropout(
    N_sim, 
    ...
    sample_size=512,device='cpu',sigmoid=False, dropout_prop=0.2
):
    """
    ...
    for sim in range(N_sim):
        ...
        engressor = engression(
            ...
            batch_size=batch_size, device=device, sigmoid=sigmoid, dropout_prop=dropout_prop
        )
